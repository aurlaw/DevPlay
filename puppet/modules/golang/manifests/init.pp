class golang ( $version = "1.4.1" ) {
  ...
  exec { "setup-path":
    command => "/bin/echo 'export PATH=/vagrant/bin:/usr/local/go/bin:/vagrant/go/bin:\$PATH' >> /home/vagrant/.profile",
    unless  => "/bin/grep -q /usr/local/go /home/vagrant/.profile ; /usr/bin/test $? -eq 0"
  }

  exec { "setup-workspace":
    command => "/bin/echo 'export GOPATH=/vagrant/go' >> /home/vagrant/.profile",
